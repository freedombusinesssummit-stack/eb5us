    ...
    <section id="audience" style={{ background: C.bgSection, padding: "72px 40px" }}>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          {[
            { flag: "🇮🇳", country: "India", note: "EB-5 #1 demand source", pct: "34%" },
            { flag: "🇻🇳", country: "Vietnam", note: "High-intent SEA market", pct: "18%" },
            { flag: "🇦🇪", country: "Middle East", note: "E-2 treaty angle", pct: "16%" },
            { flag: "🇲🇽", country: "LATAM", note: "E-2 dominant corridor", pct: "20%" },
            { flag: "🇨🇳", country: "China", note: "EB-5 post-backlog", pct: "12%" },
          ].map(({ flag, country, note, pct }) => (
            <Card key={country} style={{ textAlign: "center", padding: "20px 14px" }}>
              <div style={{ fontSize: 32, marginBottom: 10 }}>{flag}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginBottom: 4 }}>{country}</div>
              <div style={{ fontSize: 20, fontWeight: 900, color: C.green, marginBottom: 4 }}>{pct}</div>
              <div style={{ fontSize: 11, color: C.textGray }}>{note}</div>
            </Card>
          ))}
        </div>
      </div>
...
function Pricing() {
  const tiers = [
    { name: "Intelligence", price: "$1,500", tag: "Entry", features: ["Panel seat — 30 min", "Logo on event page", "Survey highlight data", "1 email broadcast", "Post-event analytics report", "Session attendee list"], cta: "Claim Slot" },
    { name: "Growth", price: "$3,500", tag: "Speaking", features: ["30-min keynote session", "Exclusive category lock", "Survey opt-in leads", "2 email broadcasts", "Session attendee list", "Post-event analytics report"], cta: "Claim Slot" },
